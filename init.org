...
#+begin_src emacs-lisp
  (defun ma-tab-name-format (tab tab-num)
    (let* ((current-p (eq (car tab) 'current-tab))
           ...
           (prefix (cond ((string-equal name "Mail")     (all-the-icons-octicon "mail")                 )
                         ...
                         ((string-equal name "VTerm")    (all-the-icons-icon-for-mode 'vterm-mode)      )
                         (t ""))))
    ...
    (when work-linux-remote
      ...
      (use-package mu4e
        :straight (:branch "release/1.8"
                           :pre-build ("perl" "-pi" "-e" "s/2\.58/2.56/" "*"))
