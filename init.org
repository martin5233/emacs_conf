...
#+begin_src emacs-lisp
  (defun ma-deadgrep-root-function()
    ...
    (if (and (buffer-file-name) (file-in-directory-p (buffer-file-name) "/scratch/apel/new_arch/develop/src/ooa"))
      ...
      (if (and (buffer-file-name) (file-in-directory-p (buffer-file-name) "/scratch/apel/new_arch/develop/src/postproc"))
          "/scratch/apel/new_arch/develop/src/postproc"
...
#+begin_src emacs-lisp
  ...
  (use-package devdocs
    ...
    (add-hook 'c-mode-common-hook
              (lambda()
                (setq-local devdocs-current-docs '("cpp" "qt"))
...
#+begin_src emacs-lisp
  ...
  (defengine duckduckgo-ff
             "https://duckduckgo.com/?q=%s"
             :keybinding "D")
  (defengine duckduckgo-eww
             "https://duckduckgo.com/?q=%s"
...
#+begin_src emacs-lisp
  (when work-linux-remote
    ...
    (defcustom ma-refile-sender-alist nil "Alist to map senders to a refile folder in mu4e"
      :group 'ma
      :type '(repeat (cons string string)))

    (defun ma-refile-folder-function(msg)
      "Function to return a refile folder for the MSG."
      (let ((sender (mu4e-contact-name (car-safe (mu4e-message-field msg :from)))))
        (dolist (entry ma-refile-sender-alist)
          (when (string-match (car entry) sender)
            (cl-return (cdr entry))))))
...
#+begin_src emacs-lisp
  ...
  (use-package org
    ...
    (org-babel-load-languages (quote ((emacs-lisp . t) (dot . t) (ditaa . t) (shell . t) (sql . t))))
...
#+begin_src emacs-lisp
  (use-package lsp-mode
    ...
    (lsp-client-packages '(lsp-bash lsp-clangd lsp-clients lsp-dockerfile lsp-groovy lsp-javascript lsp-json lsp-php lsp-pyls lsp-xml lsp-yaml lsp-ansible))
  ...
  (defun ma-lsp-ansible-check-mode (&rest _)
    "Check whether we are possibly in an ansible buffer."
    (when (and (or (derived-mode-p 'yaml-mode)
             (derived-mode-p 'yaml-ts-mode)))
      (let ((patterns '("/handlers/*.yml"
                       "/tasks/*.yml"
                       "/roles/*.yml"
                       "/vars/*.yml"
                       "/meta/*.yml"
        ...
        (seq-filter match patterns))))

  (with-eval-after-load "lsp-ansible"
    (advice-add #'lsp-ansible-check-ansible-minor-mode :override #'ma-lsp-ansible-check-mode))
...
#+begin_src emacs-lisp
  (defun ma-lint-Jenkinsfile ()
    ...
    (interactive)
    (shell-command-on-region (point-min) (point-max)  "ssh -p 9002 mal1@spck-simdev declarative-linter" (get-buffer-create "*Jenkins Lint Output*"))
    )
    (use-package jenkinsfile-mode
    :bind
    ("C-c C-c" . ma-lint-Jenkinsfile))
#+end_src
** Elisp
*** Auto-compile
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package casual
    :straight (:type git :flavor melpa :host github :repo "kickingvegas/Casual"))
#+end_src
* Martin's specials
#+begin_src emacs-lisp
